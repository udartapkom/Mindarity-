...
// Типы для API (аналогично web версии)
...
class ApiService {
  ...
  constructor() {
    this.api = axios.create({
      baseURL: 'https://mindarity.ru/api',
      headers: {
        'Content-Type': 'application/json',
        'User-Agent': `MindarityMobile/${Platform.OS}/${Platform.Version}`,
      },
      timeout: 30000,
    });
    ...
    this.api.interceptors.response.use(
      ...
      async (error) => {
        if (error.response?.status === 401) {
          // Токен истек, очищаем AsyncStorage
  ...
  // Big Data
  async uploadBigDataFile(file: any): Promise<any> {
    const formData = new FormData();
    formData.append('file', file);
